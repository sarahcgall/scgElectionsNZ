...
upload <- function(year, file, row_no){
  tmp <- read.csv(paste0("data-raw/vote-data", "/", year, "/", file, ".csv"), skip = row_no,
...
amend_parties <- function(df, column) {
  tmp <- df %>%
    mutate({{ column }} := case_when(
      {{ column }} %in% c("X99 MP Party", " 99 MP Party") ~ "99 MP Party",
      {{ column }} %in% c("ACT", "ACT New Zealand", " ACT New Zealand") ~ "ACT Party",

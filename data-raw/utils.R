...
amend_parties <- function(df, column) {
  tmp <- df %>%
    ...
    mutate({{ column }} := case_when(
      ...
      {{ column }} == "Aoteroa Legalise Cannabis Party" ~ "Aoteroa Legalise Cannabis Party",
      ...
      {{ column }} %in% c("The Libertarianz","Libertariannz") ~ "Libertarianz",
      ...
      {{ column }} %in% c("Outdoor Rec  Nz", "Outdoor Rec. NZ")  ~ "Outdoor Rec NZ",
      {{ column }} == "Others" ~ "Other",
      {{ column }} == "RAM   Residents Action Movement" ~ "RAM - Residents Action Movement",
      {{ column }} == "NZ Superannutants and Youth Action" ~ "NZ Superannuitants and Youth Action",
